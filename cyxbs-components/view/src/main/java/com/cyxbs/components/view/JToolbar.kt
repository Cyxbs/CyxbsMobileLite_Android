package com.cyxbs.components.view
...
class JToolbar(context: Context, attrs: AttributeSet?) : Toolbar(context, attrs) {
  ...
  fun init(
    ...
    @DrawableRes
    icon: Int = com.cyxbs.components.config.R.drawable.config_ic_back,
  ...
  private fun tryInitTitleTextView(child: View?): Boolean {
    if (child is TextView && child.text == title) {
      ...
      child.setTextColor(ContextCompat.getColor(context,
        com.cyxbs.components.config.R.color.config_level_two_font_color))
      return true
    }
    return false
  }
