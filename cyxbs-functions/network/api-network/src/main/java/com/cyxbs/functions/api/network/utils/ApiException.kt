...
open class ApiExceptionResult<Emitter>(
  ...
  /**
   * 只抓取 [ApiException] 异常
   * @param errorCode 单独处理 [errorCode] 对应的情况。如果传入 -1，则处理全部 errorCode
   */
  inline fun ApiException(errorCode: Int = -1, action: Emitter.(ApiException) -> Unit): KClass<ApiException> {
    return ApiException::class.catch {
      if (errorCode == -1) action.invoke(this, it)
      else if (it.errorCode == errorCode) action.invoke(this, it)
    }
  }
  ...
  ): KClass<HttpException> = HttpException::class.fromJson(httpCode, action)
   ...
   * @param httpCode 想要处理的 Http 状态码
   */
  inline fun <reified T : Any> KClass<HttpException>.fromJson(
    httpCode: Int,
    action: Emitter.(T) -> Unit
  ): KClass<HttpException> {
    errorBody(httpCode) {
      try {
        action.invoke(emitter, Json.decodeFromString(it.toString()))
